...
class SCTransFeatureExtractor:
    ...
    def extract_features(self, log_path: str):
        ...
        try:
            ...
            holistic_features = self._extract_holistic_features(ego_df, log, map_name)
    ...
    def _extract_holistic_features(self, ego_df: pd.DataFrame, log: MetricsLog, map_name: str) -> dict:
        ...
        ego_df['yaw_rate'] = np.gradient(np.unwrap(ego_df['yaw'], period=360), ego_df.index)
        ...
        features['num_other_vehicles'] = len([vid for vid in vehicle_ids if vid!= log.get_ego_vehicle_id()])
        ...
        features['collision_occurred'] = 1 if log.get_collision_events(log.get_ego_vehicle_id()) else 0
